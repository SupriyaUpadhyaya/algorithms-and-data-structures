...
class MyBinaryTree(BinaryTree):
    ...
    def max_sum(self) -> int:
        ...
        if self.get_left() and self.get_right() is not None:
            sum += self._item + self.get_left().max_sum() + self.get_right().max_sum()
        elif self.get_right() is not None:
            sum += self._item + self.get_right().max_sum()
        elif self.get_left() is not None:
            sum += self._item + self.get_left().max_sum()
        else:
            sum += self._item
        return sum
    ...
    def max_path(self) -> int:
        ...
        if self.get_item() is None:
            ...
            left = self.get_left().max_path()
            right = self.get_right().max_path()
        elif self.get_left() is not None:
            left = self.get_left().max_path()
        elif self.get_right() is not None:
            right = self.get_right().max_path()
    ...
    def find_max(self) -> int:
        ...
        if self.get_item() is None:
            return 0
        if self.get_left() and self.get_right() is not None:
            l = self.get_left().max_path()
            r = self.get_right().max_path()
        elif self.get_left() is not None:
            l = self.get_left().max_path()
            r = 0
        elif self.get_right() is not None:
        ...
        max_single = max(max(l, r) + self.get_item(), self.get_item())

        max_top = max(max_single, l + r + self.get_item())

        sum = max(max_single, max_top)
...
if __name__ == "__main__":
    ...
    tree1 = MyBinaryTree(0, MyBinaryTree(None), MyBinaryTree(46))
